...
public final class SwiftRegex {
    ...
    public static let wordRegexTypes = [
        SwiftRegexType(with: httpRegex, isExpression: false),
        SwiftRegexType(with: phoneRegex, isExpression: false),
        SwiftRegexType(with: expressionRegex, isExpression: true)
    ]
}
...
extension SwiftRegex {
    ...
    public static func regex(of text: String, textColor: UIColor = UIColor.black,textFont: UIFont = UIFont.systemFont(ofSize: 15),  lineSpaceing: CGFloat = 4, wordSpaceing: CGFloat = 0, wordRegexTypes: [SwiftRegexType] = wordRegexTypes) -> NSAttributedString? {
        if text.count <= 0 {
            return nil
        }
        ...
        for wordRegexType in wordRegexTypes {
            if wordRegexType.isExpression { // Expression matching
                ...
                { (captureCount, capturedStrings, range) in
                    ...
                    let key = NSAttributedString.Key(rawValue: SwiftRegex.imageLinkConst)
                ...
                { (captureCount, capturedStrings, range) in
                    ...
                    let key = NSAttributedString.Key(rawValue: SwiftRegex.textLinkConst)
                    attributedStr.addAttribute(key, value: capturedStrings, range: range)
                }
            }
        }
