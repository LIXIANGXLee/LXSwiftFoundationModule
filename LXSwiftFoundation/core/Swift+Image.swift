...
extension SwiftBasics where Base: UIImage {
    ...
    public func imageByRound(with radius: CGFloat) -> UIImage? {
        imageByRound(with: radius, corners: UIRectCorner.allCorners)
    }
...
extension SwiftBasics where Base: UIImage {
    ...
    public var fixOrientation: UIImage? {
        guard let bgImage = base.cgImage else {
            return base
        }
        ...
        guard let space = bgImage.colorSpace else {
            return nil
        }
