...
@objcMembers open class SwiftPickerView: UIView, SwiftPickerViewCommomDelegate {
    ...
    open var tHeaderViewTopCornerRadii: CGFloat? {
        didSet {
            guard let tHeaderView = self.tHeaderView else {
                return
            }
    ...
    open var tHeaderView: UIView? {
        didSet {
            guard let tHeaderView = tHeaderView else {
                return
            }
            setCornerRadii(tHeaderView, isHeader: true)
    ...
    open var tFooterViewBottomCornerRadii: CGFloat? {
        didSet {
            guard let tFooterView = self.tFooterView else {
                return
            }
    ...
    open var tFooterView: UIView? {
        didSet {
            guard let tFooterView = tFooterView else {
                return
            }
            setCornerRadii(tFooterView, isHeader: false)
    ...
    private lazy var tableView: SwiftTableView = {
         let rect = CGRect(x: 0,
                           ...
                           width: SCREEN_WIDTH_TO_WIDTH,
                           height: maxHeight)
         let tableView = SwiftTableView(frame: rect, style: self.style)
...
extension SwiftPickerView: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        dataSource!.pickerView(self, numberOfRowsInSection: section)
    }
    
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        dataSource!.pickerView(self, tableView: tableView, cellForRowAt: indexPath)
    }
