...
@objcMembers open class SwiftHyperlinksModalController: SwiftModalController {
    ...
    private lazy var textLabel: SwiftTextLable = {
       ...
       let textLabel = SwiftTextLable(config: config)
...
extension SwiftHyperlinksModalController {
    ...
    public func getAttributedString(with text: String, textColor: UIColor = UIColor.lx.color(hex: "666666"), textFont: UIFont = UIFont.systemFont(ofSize: 14), regexTypes: [SwiftRegexType]) -> NSAttributedString? {
        SwiftRegex.regex(of: text, textColor: textColor, textFont: textFont, wordRegexTypes: regexTypes)
    }
...
@objcMembers open class SwiftItem: NSObject {
   ...
   public init(title: String, titleColor: UIColor = UIColor.black, titleFont: UIFont = UIFont.lx.font(withMedium: 16), callBack: SwiftItem.LXSwiftModalItemCallBack?) {
        self.title = title
