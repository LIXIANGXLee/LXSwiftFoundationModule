...
@objc public protocol LXSwiftPickerViewDelegate: AnyObject {
...
@objc public protocol LXSwiftPickerViewDataSource: AnyObject {
...
public class LXSwiftPickerView: UIView, LXSwiftPickerViewCommomDelegate {
    ...
    /// 设置整体圆角 上、下、左、右
    public var setTViewAllCornerRadii: CGFloat? {
        didSet {
            guard let cornerRadii = self.setTViewAllCornerRadii else { return }
    ...
    public var setTHeaderViewTopCornerRadii: CGFloat? {
        didSet {
    ...
    public var setTFooterViewBottomCornerRadii: CGFloat? {
        didSet {
...
extension LXSwiftPickerView {
    ...
    /// 展示pickerView
    public func show(_ rootView: UIView? = nil, completion:(() -> Void)? = nil) {
        if rootView != nil {
    ...
    private func setCornerRadii(_ view: UIView, isHeader: Bool) {
        if isHeader {
            /// 判断 如果有 setContentHeaderTopCornerRadii 则设置原角
             if let cornerRadii = setTHeaderViewTopCornerRadii {
                view.lx.setPartCornerRadius(radius: cornerRadii, roundingCorners: [.topLeft,.topRight])
             }
        }else{
            if let cornerRadii = setTFooterViewBottomCornerRadii {
